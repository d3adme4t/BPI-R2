...
if [[ -z $(cat /proc/cpuinfo | grep -i 'model name.*ArmV7') ]]; then
	if [[ -z "$(which armv7a-hardfloat-linux-gnueabi-gcc)" ]];then echo "please install gcc-arm-linux-gnueabihf";exit 1;fi

#	CCVER=$(arm-linux-gnueabihf-gcc --version |grep arm| sed -e 's/^.* \([0-9]\.[0-9-]\).*$/\1/')
#	CCVER=$(armv7a-hardfloat-linux-gnueabi-gcc --version |grep arm| sed -e 's/^.* \([0-9]\.[0-9-]\).*$/\1/')
#	if [[ $CCVER =~ ^7 ]]; then
#		echo "arm-linux-gnueabihf-gcc version 7 currently not supported";exit 1;
#	fi
	export ARCH=arm;export CROSS_COMPILE=armv7a-hardfloat-linux-gnueabi-
	crosscompile=1
fi;
